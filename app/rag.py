...
def build_retrieval_plan(active_control: dict) -> dict:
    """
    Runtime-only retrieval contract.
    DO NOT store this in schema.
    """
    section = active_control["section"]
    synonyms = active_control.get("synonyms", [])

    return {
        # Canonical semantic intent
        "primary_query": section,

        # Recall probes (NO instructional text)
        "recall_queries": [s for s in synonyms if s != section],

        # ICH is rule-based → stable
        "ich_query": section,

        # Source is evidence-based → concrete
        "source_primary_query": section,
        "source_recall_queries": [s for s in synonyms if s != section],
...
def dedupe_chunks(chunks: List[Dict], key_fields=("doc_id", "page_numbers", "text")) -> List[Dict]:
    """
    Dedupe chunks deterministically.
    Prefers source_block_ids if present, else falls back to (doc_id, page_numbers, first 200 chars).
    """
    seen = set()
    out = []
    ...
    for c in chunks:
        ...
        # Best: stable block id if you ever add it
        block_ids = c.get("source_block_ids") or []
        if block_ids:
            k = ("block", block_ids[0])
        else:
            doc_id = c.get("doc_id", "")
...
def build_source_filter(active_control: dict) -> str | None:
    """
    Turn allowed_sources into Azure AI Search filter.
    If your index field is doc_id (as per your select), this will work.
    """
    allowed = active_control.get("allowed_sources") or []
    allowed = [a for a in allowed if isinstance(a, str) and a.strip()]

    if not allowed:
        return None

    if len(allowed) == 1:
        return f"doc_id eq '{allowed[0]}'"

    or_clause = " or ".join([f"doc_id eq '{a}'" for a in allowed])
    return f"({or_clause})"
...
def vector_search_tables(
    ...
    """
    Retrieve TABLE blocks using their semantic context fields.
    """
    q_vec = batch_embed([query])[0]
...
def retrieve_narrative_chunks(
    ...
    return [
        dict(r)
        for r in results
        if r.get("chunk_type") in ("paragraph", "heading")
    ]




def retrieve_tables_vector(
    search_client,
    query: str,
    filter_expr: str | None,
    k: int = 40,
...
def retrieve_tables_keyword(
    ...
    for tid in table_ids:
        ...
        out.extend(dict(r) for r in results if r.get("chunk_type") == "table")
...
def vector_search_source(search_client, query, k_nearest_neighbors=100, filter_expr=None):
    ...
    vector_query = VectorizedQuery(
        vector=q_vec,
        fields="vector",
        k=k_nearest_neighbors,  # IMPORTANT
    )
...
def retrieve_context_node(state: RAGState) -> RAGState:
    ...
    # If not found, fallback control (your original behavior)
    ...
    # -------------------------------------------------
    # BUILD RETRIEVAL PLAN (NEW)
    # -------------------------------------------------
    retrieval_plan = build_retrieval_plan(active_control)

    # -------------------------------------------------
    # ICH RETRIEVAL (RULES ONLY)
    # - ONE clean query (no "also look for", no table ids)
    # -------------------------------------------------
    ich_context = "No ICH guidance found."
    ich_chunks = []

    ich_refs = retrieval_plan.get("ich_refs") or []
    if ich_refs and isinstance(ich_refs, list) and ich_refs[0]:
        ich_client = load_ich_search_client()

        # Build ICH filter from ich_refs[0] the way you already do
        section_number, section_text = split_section(ich_refs[0])
        ich_filter_expr = None
        ...
        ich_query = retrieval_plan.get("ich_query") or section_name

        ich_chunks = vector_search_ich(
            ich_client,
            ich_query,
            k_nearest_neighbors=80,
            filter_expr=ich_filter_expr,
        )
    ...
    # - Primary + recall queries separately (vector search friendly)
    # -------------------------------------------------
    source_client = load_source_search_client()

    source_filter_expr = build_source_filter(active_control)
    print("Source filter expr : ",source_filter_expr)

   # -------------------------------------------------
    # SOURCE RETRIEVAL (BLOCK-AWARE)
    # -------------------------------------------------

    source_filter_expr = build_source_filter(active_control)

    # Narrative (paragraphs + headings)
    narrative_chunks = retrieve_narrative_chunks(
        source_client,
        query=retrieval_plan["primary_query"],
        filter_expr=source_filter_expr,
    )

    # Tables (semantic)
    table_chunks_vector = retrieve_tables_vector(
        source_client,
        query=retrieval_plan["primary_query"],
        filter_expr=source_filter_expr,
    )

    # Tables (deterministic by ID)
    table_chunks_keyword = retrieve_tables_keyword(
        source_client,
        table_ids=[
            t for t in retrieval_plan["source_recall_queries"]
    ...
    print("Source chunks (final):", len(source_chunks))
...
""".strip()
    ...
    # Debug prints (keep while stabilizing)
    print("=== Retrieval debug ===")
    print("Section:", section_name)
    print("ICH chunks:", len(ich_chunks))
    print("Source chunks deduped:", len(source_chunks))
